...
export async function showNotification(options: NotificationOptions) {
	try {
		...
		const cmd = `${alerter} -message "${options.content}" -title "${options.title}" ${options.group ? `-group "${options.group}"` : ''} -sender "ch.origaming.appleblox" ${options.timeout ? '-timeout ' + Math.floor(options.timeout) : ''
